...
const Button = (props) => {
  return (
    <div onClick={props.onClick}
      id={props.data.id}
      className={["button", props.data.class].join(" ")}>
      {props.data.inner}
    </div>
  );
}
...
const Calculator = () => {
  ...
  const handleClick = (e) => {
    ...
    if (data.input.includes('=')) {
      ...
      else newInput += e.target.innerText;
    }
    ...
    else newResult = newResult[newResult.length - 1];
  ...
  const Buttons = buttonsProps.map(props => <Button data={props} onClick={handleClick} />);
